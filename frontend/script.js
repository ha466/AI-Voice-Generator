...
class AudioHistoryManager {
    ...
    updateHistoryUI() {
        ...
        historyList.innerHTML = this.history.map(item => `
            <div class="history-item" data-id="${item.id}" onclick="audioHistoryManager.loadHistoryItem('${item.audioUrl}')">
...
async function generateAudio() {
    ...
    try {
        ...
        if (audioPlayer && audioPlayerContainer) {
            audioPlayer.src = `/audio/${data.filename.split('/').pop()}`;
            audioPlayerContainer.style.display = 'block';
        }
        
        showFeedback('success');
        const selectedVoice = voiceSelect.options[voiceSelect.selectedIndex].text;
        audioHistoryManager.addToHistory(textInput.value, audioPlayer?.src, selectedVoice);
